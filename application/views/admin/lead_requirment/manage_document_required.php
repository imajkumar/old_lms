...
<div id="wrapper">
									...
									<td><a href="#" onclick="edit_status(this,<?php echo $status['id']; ?>);return false;" data-color="<?php echo $status['color']; ?>" data-name="<?php echo $status['name']; ?>" data-code="<?php echo $status['code']; ?>"><?php echo $status['name']; ?></a><br />
										...
										<td>
											...
											<?php											
												if($status['status'] == 1){
											?>
												<a href="<?php echo admin_url('leads/document_required_status_inactive/'.$status['id']); ?>" class="btn btn-danger"><i class="fa fa-ban"></i></a>
											<?php }else{ ?>
												<a href="<?php echo admin_url('leads/document_required_status_active/'.$status['id']); ?>" class="btn btn-danger"><i class="fa fa-check"></i></a>
	...
	<?php include_once(APPPATH.'views/admin/lead_requirment/document_required.php'); ?>
	<?php init_tail(); ?>
</body>
</html>
