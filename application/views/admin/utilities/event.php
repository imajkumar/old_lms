   <div class="modal fade _event" id="viewEvent">
        ...
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
			  ...
			  <?php 
				$lead_list = $this->utilities_model->list_leads_data(); 
				$s_attrs['disabled'] = 'disabled';
				$selected = array();
				array_push($selected,$event->leadid);
			 ?>
			 <div class="form-group" app-field-wrapper="description">
			 <label for="description" class="control-label">Lead *</label>

			  <?php echo render_select('dummyleadid',$lead_list,array('id','company'),'',$selected,array('data-width'=>'100%','data-none-selected-text'=>_l('leads')),$s_attrs); ?>
			  <?php echo form_hidden('leadid',$event->leadid); ?>
			 </div>
			
              <?php echo render_input('title','Meeting Title',$event->title); ?>
              <?php echo render_textarea('description','Meeting Description',$event->description,array('rows'=>5)); ?>
              <?php echo render_datetime_input('start','Meeting Date',_dt($event->start)); ?>
              <div class="clearfix mtop15"></div>
              <?php echo form_hidden('end','utility_calendar_new_event_end_date',_dt($event->end)); ?>
    ...
    <div class="modal-footer">
      ...
      <?php if($event->userid == get_staff_user_id() || is_admin()){ ?>
      <button type="button" class="btn btn-danger hide" onclick="delete_event(<?php echo $event->eventid; ?>); return false"><?php echo _l('delete_event'); ?></button>
...
<script>
    $(document).ready(function(){
        $("#dummyleadid").attr('disabled', 'disabled');
    });
</script>