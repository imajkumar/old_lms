...
?>
<script>
$("input[name='onoffswitchis_primary']").click(function(){
...
$(document).on('change', '#country_id', function (e) {
   ...
   $("#name").keyup(function () {
        ...
        $.ajax({
            type: "POST",
            url: admin_url + 'clients/GetGroupName',
            data: {
                keyword: $("#name").val()
            },
            dataType: "json",
	...
	$("#company").keyup(function () {
        ...
        $.ajax({
            type: "POST",
            url: admin_url + 'clients/GetCompanyName',
            data: {
                keyword: $("#company").val()
            },
            dataType: "json",
            success: function (data) {
                ...
                if (data.length > 0) {
                    ...
                    $('#company').attr("data-toggle", "dropdown");
                    $('#Dropdowncompany').dropdown('toggle');
                }
                else if (data.length == 0) {
                    $('#company').attr("data-toggle", "");
					$('#Dropdowncompany').hide();
                }
                $.each(data, function (key,value) {
                    if (data.length >= 0)
                        $('#Dropdowncompany').append('<li role="displaycompany" ><a role="menuitem dropdowncompanyli" class="dropdownlivalue">' + value['company'] + '</a></li>');
                });
