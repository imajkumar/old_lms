						...
						<table class="collaptable table table-bordered">
						  ...
						  <?php
							...
							$query_rsm= $this->db->query("SELECT staffid,emp_code,firstname,lastname FROM tblstaff where role=2 AND reporting_manager IN ('". $chkdynsm ."') ORDER BY staffid ASC");
								...
								$get_opportunity_sum_rsm += get_opportunity_sum_rsm($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
						   ...
						   ?>
							  ...
							  <?php
								...
								if($filter=='this_month'){
									$month = date('Y-m');
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_month'){
									$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='this_year'){
									$year = date('Y');
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_year'){
									$year = date('Y', strtotime(date('Y')." -1 year"));
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='custom'){
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND (dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )  AND status NOT IN(6,7)');
								}else{
									$query_rsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_rsm['staffid'].'" AND status NOT IN(6,7)');
								}
								$result4res_rsm = $query_rsm->result_array();
								$total_chkse=$totasm;
								$totse = 200;
								foreach($result4res_rsm as $res4res_rsm){
								
						   ?>
							  <tr style="display: table-row;background: #99dde8; font-size: 14px;" data-id="<?php echo $res_rsm['staffid']; ?>" data-parent="5">
								<td style="text-align: left !important;width:220px;"></td>
								<td><?php echo $res4res_rsm['opportunity']; ?></td>
								<td><?php echo $res4res_rsm['region']; ?></td>
								<td><?php echo $this->leads_model->get_city_name($res4res_rsm['city']); ?></td>
								<td><?php echo $res4res_rsm['id']; ?></td>
								<td><?php echo $res4res_rsm['dateadded']; ?></td>
								<td><?php echo $this->leads_model->get_customer_name($res4res_rsm['customer_name']); ?></td>
								<td><?php echo $res4res_rsm['customer_type']; ?></td>
								<td><?php 
									
										$CAT = $this->leads_model->get_product_description($res4res_rsm['id']);
										$string = '';
								...
								<td><?php echo $this->leads_model->get_status_name($res4res_rsm['status']); ?></td>
								<td><?php $timestamp = strtotime($res4res_rsm['accepacted_date']);
									$date = date("d-m-Y", $timestamp);
									echo $date; ?></td>
								<td><?php
								 	echo $res4res_rsm['status_lost'];
						   ...
						   ?>
							...
							<?php
								...
								if($filter=='this_month'){
									$month = date('Y-m');
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_month'){
									$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='this_year'){
									$year = date('Y');
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_year'){
									$year = date('Y', strtotime(date('Y')." -1 year"));
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='custom'){
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND (dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )  AND status NOT IN(6,7)');
								}else{
									$query_zsm = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND status NOT IN(6,7)');
								}
								$result4res_zsm = $query_zsm->result_array();
								$total_chkse=$totasm;
								$totse = 200;
						   ...
						   ?>
							   <tr data-id="<?php echo $res_se['staffid']; ?>" data-parent="<?php echo $chkasm_id; ?>">
								...
								<td><?php $timestamp = strtotime($res4['accepacted_date']);
									$date = date("d-m-Y", $timestamp);
									echo $date; ?></td>
								<td><?php
							...
							<tr style="text-align:center;background: #ff9201;">
								...
								<td><?php echo $get_opportunity_sum_rsm; ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
