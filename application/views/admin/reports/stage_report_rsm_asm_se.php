         ...
         <div class="panel_s accounting-template estimate">
            <div class="panel-body">
               <div class="row">
                  <div class="col-md-12">
					...
					<div class="col-md-2 leads-filter-column ">
					  <h5>Zone:</h5>
						 ...
						 ?>
						<select <?php if(get_staff_role() == 1 || get_staff_role() == 3 || get_staff_role() == 5){ echo 'disabled'; } ?>  id="region_id" name="region_id" class="form-control">
					...
					<?php 
							if($this->input->post('region_id') != '--Select--'){
								$arruser = "region LIKE('%".$this->input->post('region_id')."%') AND ( CONCAT(',', reporting_to, ',')  LIKE '%, ".get_staff_user_id().",%'  OR CONCAT(',', reporting_to, ',')  LIKE '%,".get_staff_user_id().",%' ) AND is_not_staff = 0";
							}else{
								$arruser = "( CONCAT(',', reporting_to, ',')  LIKE '%, ".get_staff_user_id().",%'  OR CONCAT(',', reporting_to, ',')  LIKE '%,".get_staff_user_id().",%' ) AND is_not_staff = 0";
							}
				  ...
				  <div class="col-md-12">
                     <br>
                     <table class="table <?php if($this->input->post('from-region')=='pan_india'){ echo 'hidden'; } ?>"  style="text-align:center;">
                        ...
                        <tbody>
                           ...
                           <?php
							...
							if($this->input->post('from-months') !=''){
								...
								if($filtermonth-1 > 9){
									$thisMonth = $filteryear.'-'.($filtermonth-1);
								}else{
									$thisMonth = $filteryear.'-0'.($filtermonth-1);
									
								}
								if($filtermonth-2 > 9){
									$lastMonth = $filteryear.'-'.($filtermonth-2);	
								}else{
									$lastMonth = $filteryear.'-0'.($filtermonth-2);	
								}
								if($filtermonth-3 > 9){
									$last2Month = $filteryear.'-'.($filtermonth-3);
								}else{
									$last2Month = $filteryear.'-0'.($filtermonth-3);
						...
						<tr>
							...
							<th style="text-align:center;" bgcolor="#f4b084" colspan="3"><strong><?php echo date("M", mktime(0, 0, 0, $filtermonth-3, 10)).'-'.date('Y'); ?></strong></th>
							<th style="text-align:center;" bgcolor="#f4b084" colspan="3"><strong><?php echo date("M", mktime(0, 0, 0, $filtermonth-2, 10)).'-'.date('Y') ;?></strong></th>
							<th style="text-align:center;" bgcolor="#f4b084" colspan="3"><strong><?php echo date("M", mktime(0, 0, 0, $filtermonth-1, 10)).'-'.date('Y') ;?></strong></th>
					...
					<tbody>
						...
						<tr>
						 ...
						 <tr bgcolor="#f4b084">
							...
							<td width='100px'>Lead Value</td>
							<td width='100px'>Risk Weighted</td>
							<td width='100px'>Lead</td>
							<td width='100px'>Lead Value</td>
							<td width='100px'>Risk Weighted</td>
							<td width='100px'>Lead</td>
					...
					<?php 
							...
							$leadstage = $this->db->get('tblleadsstatus')->result_array();
                              foreach($leadstage as $data_l)
                              {
								...
								$last2Monthno_of_leadsTotal = $last2Monthno_of_leadsTotal + $this->leads_model->no_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']);
								$lastMonthno_of_leadsTotal = $lastMonthno_of_leadsTotal + $this->leads_model->no_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']);
								$thisMonthno_of_leadsTotal = $thisMonthno_of_leadsTotal + $this->leads_model->no_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']);				
								
								$leadsTotal = $this->leads_model->no_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']) + $this->leads_model->no_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']) + $this->leads_model->no_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']);
								
								$leadsTotal_of_total = $leadsTotal_of_total + $leadsTotal;
								
								$last2Monthvalue_of_leadsTotal = $last2Monthvalue_of_leadsTotal + $this->leads_model->value_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']);
								$lastMonthvalue_of_leadsTotal = $lastMonthvalue_of_leadsTotal + $this->leads_model->value_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']);
								$thisMonthvalue_of_leadsTotal = $thisMonthvalue_of_leadsTotal + $this->leads_model->value_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']);			
								
								$leadsAmountTotal = $this->leads_model->value_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']) + $this->leads_model->value_of_leads_by_stage_month_staff($lastMonth,$data_l["id"]) + $this->leads_model->value_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']);
                              ...
                              ?>
								...
								<tr>
									...
									<td><?php echo $this->leads_model->no_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']); ?></td>
									<td><?php echo $this->leads_model->value_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']) ?></td>
								...
								<?php } else{ ?>
									<td><?php echo round(($this->leads_model->value_of_leads_by_stage_month_staff($last2Month,$data_l["id"],$staff_id,$region['id']) * $data_l["weighted"]) / 100 ,0); ?></td>
								<?php } ?>
								
									<td><?php echo $this->leads_model->no_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']); ?></td>
									<td><?php echo $this->leads_model->value_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']); ?></td>
								...
								<?php } else{ ?>
									<td><?php echo round(($this->leads_model->value_of_leads_by_stage_month_staff($lastMonth,$data_l["id"],$staff_id,$region['id']) * $data_l["weighted"]) / 100,0); ?></td>
								<?php } ?>
								
									
									<td><?php echo $this->leads_model->no_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']); ?></td>
									<td><?php echo $this->leads_model->value_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']) ?></td>
								...
								<?php } else{ ?>
									<td><?php echo round(($this->leads_model->value_of_leads_by_stage_month_staff($thisMonth,$data_l["id"],$staff_id,$region['id']) * $data_l["weighted"]) / 100,0); ?></td>
								<?php } ?>
