                 ...
                 <form method="post" id="form1">
				   <div class="col-md-2">
					...
					<div class="form-group" id="report-time11">
                        <label for="months-report"><?php echo _l('RSM'); ?></label><br />
                        <select class="selectpicker" id="rsm" name="rsm" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                           <option value=""><?php echo _l('--Select RSM--'); ?></option>
                           <?php
						...
						<table class="collaptable table table-bordered">
						  ...
						  <?php
							 ...
							 */
								...
								$query_zsm = $this->db->query("SELECT staffid,emp_code,firstname,lastname FROM tblstaff where role=5 AND reporting_manager IN ('". $rsm ."') ");
								$result_zsm = $query_zsm->result_array();
								foreach($result_zsm as $res_zsm){
								if(in_array($res_zsm['staffid'], $chkzsm)){
						   ?>
							   <tr style="display: table-row;background: #ff9d9df5; font-size: 14px;" data-id="<?php echo $res_zsm['staffid']; ?>" data-parent="">
...
$(document).on('change', '#rsm', function (e) {
		$('#AllZSM').html("");
		...
		$.ajax({
            ...
            success: function (data) {
                ...
                {
                    div_data += "<input type='checkbox' name='chkzsm[]' class='chkzsm' value=" + obj.staffid + ">" + obj.firstname +' '+ obj.lastname + "<br>";					
					string_dynsm += obj.staffid+',';
                });
				get_asm(string_dynsm);
				console.log(string_dynsm);
				 $('#AllZSM').append(div_data);
