<script>
 ...
 var report_pipeline = $('#pipeline-report');
 var report_winloss = $('#winloss-report');
 ...
 $(function() {
   ...
   $('.table-pipeline-report').on('draw.dt', function() {
     var invoiceReportsTable = $(this).DataTable();
     var sums = invoiceReportsTable.ajax.json().sums;
     add_common_footer_sums($(this),sums);
     $(this).find('tfoot td.amount_open').html(sums.amount_open);
 ...
 function init_report(e, type) {
   ...
   report_pipeline.addClass('hide');
   report_winloss.addClass('hide');
   ...
   // Clear custom date picker
       ...
       } else if (type == 'pipeline-report') {
         report_pipeline.removeClass('hide');
       }else if (type == 'winloss-report') {
         report_winloss.removeClass('hide');
   ...
   function pipeline_report() {
     if ($.fn.DataTable.isDataTable('.table-pipeline-report')) {
       $('.table-pipeline-report').DataTable().destroy();
     }
     initDataTable('.table-pipeline-report', admin_url + 'reports/pipeline', false, false, fnServerParams, [0, 'ASC']);
   }
 ...
 function winloss_report(){
   if ($.fn.DataTable.isDataTable('.table-winloss-report')) {
     $('.table-winloss-report').DataTable().destroy();
   }
   initDataTable('.table-winloss-report', admin_url + 'reports/winloss_report', false, false, fnServerParams, [0, 'ASC']);
 }
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
       ...
       customers_report();
     }else if (!report_winloss.hasClass('hide')) {
       winloss_report();
     }else if (!report_pipeline.hasClass('hide')) {
       pipeline_report();
