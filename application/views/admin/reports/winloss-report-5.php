						  ...
						  <?php
							...
							$query_asm = $this->db->query("SELECT staffid,emp_code,firstname,lastname FROM tblstaff where role=3 AND reporting_manager IN ('". $chkzsm ."') ");							
								...
								foreach($result_asm as $res_asm){
									...
									$get_lost_amount_sum_asm +=get_lost_amount_sum_asm($res_asm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
                                    
									$get_order_amount_sum_asm += get_order_amount_sum_asm($res_asm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
						   ...
						   ?>
							...
							<?php
								...
								$get_opportunity_sum_s = $get_opportunity_sum_s + $res4res_asm['opportunity'];
								$get_project_total_amount_sum_s = $get_project_total_amount_sum_s + $res4res_asm['project_total_amount'];
								if($res4res_asm['status']==7)
								{ 
									$get_opportunity_sums = $get_opportunity_sums + $res4res_asm['opportunity'];
								}
						   ?>
						   ...
						   ?>
							...
							<tr style="text-align:center;background: #ff9201;">
								...
								<td><?php echo $get_order_amount_sum_asm; ?></td>
								<td><?php echo $get_lost_amount_sum_asm; ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
