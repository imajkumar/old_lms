...
//-------------------- ASM -------------------------------//
 ...
 function get_opportunity_sum($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
	}else if($filter=='last_month'){
		$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
	}else if($filter=='this_year'){
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
	}else if($filter=='custom'){
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%")  AND (dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )  AND status NOT IN(6,7)');
	}else{
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%")  AND status NOT IN(6,7)');
	}
...
function get_opportunity_sum_Sales($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)'); 
	}else if($filter=='last_month'){
		$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)'); 
	}else if($filter=='this_year'){
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")   AND dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)'); 
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")   AND dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)'); 
	}else if($filter=='custom'){
		
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" ) AND status NOT IN(6,7)');
	}else{
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND status NOT IN(6,7)');
	}
	return $query->row()->total_opportunity;
 }	
						  ...
						  <?php
							...
							$data = array();
								...
								if(in_array($res_zsm['staffid'], $chkzsm)){
									$get_opportunity_sum_zsm += get_opportunity_sum_zsm($res_zsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
							?>
						   ...
						   ?>
							...
							<tr style="text-align:center;background: #ff9201;">
								...
								<td><?php echo $get_opportunity_sum_zsm; ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
