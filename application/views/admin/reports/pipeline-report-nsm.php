...
function get_opportunity_sum_dynsm($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		...
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND (dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" ) AND status NOT IN(6,7)');
		$query_staff = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as project_total_amount FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND (dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" ) AND status NOT IN(6,7)');
	}else{
...
//-------------------- ASM -------------------------------//
 ...
 function get_opportunity_sum($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		...
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" )');
	}else{
 ...
 function get_project_total_amount_sum($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		...
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" )');
	}else{
...
function get_opportunity_sum_Sales_loss($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		...
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" ) AND status=7');
	}else{
 ...
 function get_opportunity_sum_ASM_loss($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		...
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" ) AND status=7');
	}else{
                 ...
                 <form method="post" id="form1">
				   <div class="col-md-2">
					<div class="form-group" id="report-time">
                     ...
                     <div id="date-range" class="<?php if($this->input->post('months-report')=='custom'){ echo 'open'; }else { echo 'hide'; } ?> mbot15">
                        <div class="row">
                           <div class="col-md-12">
					...
					<div class="row">
						...
						<table class="collaptable table table-bordered">
						  ...
						  <?php
							...
							$result_dynsm = $query->result_array();
							$total_chkdynsm=1;
							foreach($result_dynsm as $res_dynsm){
								
								if(in_array($res_dynsm['staffid'], $chkdynsm)){
								
								$get_opportunity_sum_dynsm += get_opportunity_sum_dynsm($res_dynsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
									
						  ?>
							<tr style="display: table-row;background: #d7b9f5; font-size: 14px;" data-id="<?php echo $res_dynsm['staffid']; ?>" data-parent="">
								<td style="text-align: left !important;width:220px;"><?php echo $res_dynsm['firstname'].' '.$res_dynsm['lastname']; ?></td>
								<td><?php echo get_opportunity_sum_dynsm($res_dynsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
						   ...
						   ?>
							...
							<?php
								...
								if($filter=='this_month'){
									$month = date('Y-m');
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_month'){
									$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$month.'%") AND status NOT IN(6,7)');
								}else if($filter=='this_year'){
									$year = date('Y');
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='last_year'){
									$year = date('Y', strtotime(date('Y')." -1 year"));
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND  dateadded LIKE ("'.$year.'%") AND status NOT IN(6,7)');
								}else if($filter=='custom'){
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND (dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" )  AND status NOT IN(6,7)');
								}else{
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_zsm['staffid'].'" AND status NOT IN(6,7)');
								}
						   ...
						   ?>
							   <tr data-id="<?php echo $res_zsm['staffid']+500; ?>" data-parent="<?php echo $res_zsm['staffid']; ?>">
								...
								<td><?php $timestamp = strtotime($res4res_zsm['accepacted_date']);
									$date = date("d-m-Y", $timestamp);
									echo $date; ?></td>
								<td><?php
								 	echo $res4res_zsm['status_lost'];
						   ...
						   ?>
							...
							<?php
								...
								if($filter=='this_month'){
									...
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_asm['staffid'].'" AND (dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" )  AND status NOT IN(6,7)');
								}else{
						   ...
						   ?>
							   <tr data-id="<?php echo $res_asm['staffid']+500; ?>" data-parent="<?php echo $res_asm['staffid']; ?>">
								...
								<td><?php $timestamp = strtotime($res4res_asm['accepacted_date']);
									$date = date("d-m-Y", $timestamp);
									echo $date; ?></td>
								<td><?php
						   ...
						   ?>
							  ...
							  <?php
								...
								if($filter=='this_month'){
									...
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'" AND (dateadded BETWEEN "'.$report_from.'%" AND "'.$report_to.'%" )');
