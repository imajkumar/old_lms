...
init_head(); 
 ...
 function get_opportunity_sum($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='last_month'){
		$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='this_year'){
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='custom'){
		
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND (dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )');
	}else{
 ...
 function get_project_total_amount_sum($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$month.'%")');
		...
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='custom'){
		
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where reportingto LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )');
	}else{
 ...
 function get_opportunity_sum_s($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='last_month'){
		$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='this_year'){
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='custom'){
		
		$query = $ci->db->query('SELECT COALESCE(SUM(opportunity),0) as total_opportunity FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )');
	}else{
 ...
 function get_project_total_amount_sum_s($staff_id='',$filter='',$report_from='',$report_to=''){
	...
	if($filter=='this_month'){
		$month = date('Y-m');
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='last_month'){
		$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$month.'%")');
	}else if($filter=='this_year'){
		$year = date('Y');
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='last_year'){
		$year = date('Y', strtotime(date('Y')." -1 year"));
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%")  AND dateadded LIKE ("'.$year.'%")');
	}else if($filter=='custom'){
		
		$query = $ci->db->query('SELECT COALESCE(SUM(project_total_amount),0) as project_total_amount FROM tblleads where assigned LIKE ("%'.$staff_id.'%") AND ( dateadded BETWEEN "'.$report_from.' 00:00:00" AND "'.$report_to.' 23:59:59" )');
	}else{
...
function get_opportunity_sum_Sales_loss($staff_id='',$filter='',$report_from='',$report_to=''){
 ...
 function get_opportunity_sum_ASM_loss($staff_id='',$filter='',$report_from='',$report_to=''){
	$ci =& get_instance();
	if($filter=='this_month'){
		$month = date('Y-m');
                 ...
                 <form method="post" id="form1">
				   <div class="col-md-2">
					...
					<div class="form-group" id="report-time11">
                        <label for="months-report"><?php echo _l('DyNSM'); ?></label><br />
                        <select class="selectpicker" id="dynsm" name="dynsm" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                           <option value=""><?php echo _l('--Select DyNSM--'); ?></option>
                           <?php
					...
					<div class="row">
						...
						<table class="collaptable table table-bordered">
						  ...
						  <?php
							...
							$chkdynsm = $this->input->post('dynsm');
							/* $string_version = implode(',', $original_array)

							 */
							 
								$query_rsm= $this->db->query("SELECT staffid,emp_code,firstname,lastname FROM tblstaff where role=2 AND reporting_manager IN ('". $chkdynsm ."') ORDER BY staffid ASC");
							
								$result_rsm = $query_rsm->result_array();
								
								foreach($result_rsm as $res_rsm){
									
									$get_opportunity_sum += get_opportunity_sum($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
                                    $get_project_total_amount_sum += get_project_total_amount_sum($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
                                    $get_opportunity_sum_ASM_loss += get_opportunity_sum_ASM_loss($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to'));
								
								if(in_array($res_rsm['staffid'], $chkrsm)){
						   ?>
							   <tr style="display: table-row;background: #99dde8; font-size: 14px;" data-id="<?php echo $res_rsm['staffid']; ?>" data-parent="">
								<td style="text-align: left !important;width:220px;"><?php echo $res_rsm['firstname'].' '.$res_rsm['lastname']; ?></td>
								<td><?php echo get_opportunity_sum($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td><?php echo get_project_total_amount_sum($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td><?php echo get_opportunity_sum_ASM_loss($res_rsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
						   ...
						   ?>
							   <tr style="display: table-row;background: #ff9d9df5; font-size: 14px;" data-id="<?php echo $res_zsm['staffid']; ?>" data-parent="<?php echo $chkrsm_id; ?>">
								...
								<td><?php echo get_opportunity_sum($res_zsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td><?php echo get_project_total_amount_sum($res_zsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td><?php echo get_opportunity_sum_ASM_loss($res_zsm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
						   ...
						   ?>
							   <tr style="display: table-row;background: #fddede;font-size: 14px;" data-id="<?php echo $res_asm['staffid']; ?>" data-parent="<?php echo $chkzsm_id; ?>">
								...
								<td><?php echo get_opportunity_sum_ASM_loss($res_asm['staffid'],$this->input->post('months-report'),$this->input->post('report-from'),$this->input->post('report-to')); ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
						   ...
						   ?>
							   <tr style="display: table-row;background: rgb(230, 230, 230);font-size: 14px;" data-id="<?php echo $res_se['staffid']; ?>" data-parent="<?php echo $chkasm_id; ?>">
							  ...
							  <?php
								...
								if($filter=='this_month'){
									$month = date('Y-m');
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'" AND dateadded LIKE ("'.$month.'%")');
								}else if($filter=='last_month'){
									$month = date('Y-m' , strtotime(date('Y-m')." -1 month"));
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'" AND dateadded LIKE ("'.$month.'%")');
								}else if($filter=='this_year'){
									$year = date('Y');
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'" AND dateadded LIKE ("'.$year.'%")');
								}else if($filter=='last_year'){
									$year = date('Y', strtotime(date('Y')." -1 year"));
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'" AND dateadded LIKE ("'.$year.'%")');
								}else if($filter=='custom'){
									...
									$query4 = $this->db->query('SELECT * FROM tblleads where assigned = "'.$res_se['staffid'].'"');
								}
						   ...
						   ?>
							...
							<tr style="text-align:center;background: #ff9201;">
								...
								<td><?php echo $get_opportunity_sum_ASM_loss; ?></td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
								<td>-</td>
...
$(document).on('change', '#dynsm', function (e) {
		$('#AllRSM').html("");
		
        var dynsm_id = $(this).val();
		...
		$.ajax({
            ...
            success: function (data) {
                ...
                {
                    div_data += "<input type='checkbox' name='chkrsm[]' class='chkrsm' value=" + obj.staffid + ">" + obj.firstname +' '+ obj.lastname + "<br>";					
					string_dynsm += obj.staffid+',';
                });
				get_zsm(string_dynsm);
				console.log(string_dynsm);
				 $('#AllRSM').append(div_data);
