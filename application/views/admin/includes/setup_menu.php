<div id="setup-menu-wrapper" class="animated <?php if($this->session->has_userdata('setup-menu-open') && $this->session->userdata('setup-menu-open') == true){echo 'display-block';} ?>">
    <ul class="nav metis-menu" id="setup-menu">
        ...
        foreach ($menu_active->setup_menu_active as $item) {
            ...
            ?>
            <li>
                <a <?php if(!($item->children)){ ?> class="a-setup-menu" <?php } ?> href="<?php echo $url; ?>"><?php if(!empty($item->icon)){ ?><i class="<?php echo $item->icon; ?> menu-icon"></i><?php } ?><?php echo _l($item->name); ?>
                    <?php if($submenu == true){ ?>
                ...
                <ul class="nav nav-second-level collapse" aria-expanded="false">
                    <?php foreach($item->children as $submenu){
                       ...
                       ?>
                       <li>
                        <a  class="a-setup-menu" href="<?php echo $url; ?>">
        ...
        <li>
            <a href="<?php echo do_action('help_menu_item_link','#'); ?>" target="_blank"><?php echo do_action('help_menu_item_text',_l('setup_help')); ?></a>
        </li>
        <?php } ?>
    </ul>
</div>
