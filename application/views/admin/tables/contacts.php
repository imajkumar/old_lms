...
foreach ($rResult as $aRow) {
	...
	$row[] = '<a href="tel:'.$aRow['mobilenumber'].'">'.$aRow['mobilenumber'].'</a>';
 
	$row[] = $aRow['title'];
	
	$row[] = '<a href="mailto:'.$aRow['email'].'">'.$aRow['email'].'</a>';

   if ($aRow['is_primary'] == 1) {
    $outputActive = '<div class="onoffswitch">
                <input type="checkbox" disabled="false" data-switch-url="'.admin_url().'clients/change_contact_status" name="onoffswitch" class="onoffswitch-checkbox disabled" id="c_'.$aRow['id'].'" data-id="'.$aRow['id'].'"' . ($aRow['active'] == 1 ? ' checked': '') . '>
	...
	$outputActive1 = '<div class="onoffswitch">
                <input type="radio" data-switch-url="'.admin_url().'clients/change_contact_is_primary" name="onoffswitchis_primary" class="onoffswitch-checkbox" id="cp_'.$aRow['id'].'" data-id="'.$aRow['id'].'"' . ($aRow['is_primary'] == 1 ? ' checked': '') . '>
                <label class="onoffswitch-label" for="cp_'.$aRow['id'].'"></label>
            </div>';
