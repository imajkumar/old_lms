...
$aColumns = array(
    'tblcontacts.firstname as firstname',
    'tblcontacts.lastname as lastname',
    'tblcontacts.email as email',
    '(SELECT company FROM tblclients WHERE tblclients.userid=tblcontacts.userid) as company',
    '(SELECT reportingto FROM tblclients WHERE tblclients.userid=tblcontacts.userid) as reportingto',
	'CONCAT(tblstaff.firstname, " ", tblstaff.lastname) as assign_fullname',
    'tblcontacts.phonenumber as phonenumber',
    'tblcontacts.title as title',
    'tblcontacts.last_login as last_login',
    'tblcontacts.addedby as addedby',
...
$sTable = 'tblcontacts';
$join = array(
	'LEFT JOIN tblstaff ON tblstaff.staffid = tblcontacts.addedby',    
);
...
if(get_staff_role() == 1) 
{
	array_push($where, 'AND tblcontacts.addedby IN('. get_staff_user_id() .')');
}
 ...
 */
    ...
    if ($aRow['addedby'] !='') {
      ...
      $full_name = $aRow['assign_fullname'];
        ...
        $assignedOutput = '<a data-toggle="tooltip" data-title="'.$full_name.'" href="'.admin_url('profile/'.$aRow['addedby']).'">'.$full_name. '</a>';
