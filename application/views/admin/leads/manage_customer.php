...
<div id="wrapper">
								...
								<tr>
									<td><a href="#" onclick="edit_status(this,<?php echo $status['id']; ?>);return false;" data-color="<?php echo $status['color']; ?>" data-name="<?php echo $status['name']; ?>" data-code="<?php echo $status['code']; ?>"><?php echo $status['name']; ?></a><br />
										...
										<td>
											...
											<!--
											<a href="<?php echo admin_url('leads/delete_customer/'.$status['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-ban"></i></a>
											-->
											<?php											
												if($status['status'] == 1){
											?>
												<a href="<?php echo admin_url('leads/customer_status_active/'.$status['id']); ?>" class="btn btn-danger"><i class="fa fa-check"></i></a>
											<?php }else{ ?>
												<a href="<?php echo admin_url('leads/customer_status_inactive/'.$status['id']); ?>" class="btn btn-danger"><i class="fa fa-ban"></i></a>
	...
	<?php include_once(APPPATH.'views/admin/leads/customer.php'); ?>
	<?php init_tail(); ?>
</body>
</html>
