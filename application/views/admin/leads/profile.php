<div class="lead-wrapper" <?php if(isset($lead) && ($lead->junk == 1 || $lead->lost == 1)){ echo 'lead-is-junk-or-lost';} ?>>
   <?php if(isset($lead)){ ?>
   <div class="btn-group pull-left lead-actions-left hide">
      <!-- <a href="#" lead-edit class="mright10 font-medium-xs pull-left<?php if($lead_locked == true){echo ' hide';} ?>">
         <?php echo _l('edit'); ?>
         <i class="fa fa-pencil-square-o"></i>
      </a>
	   -->
	   <a href="<?php echo base_url('admin/leads/manage/'.$lead->id) ?>" class="mright10 font-medium-xs pull-left<?php if($lead_locked == true){echo ' hide';} ?>">
         <?php echo _l('edit'); ?>
         <i class="fa fa-pencil-square-o"></i>
      </a>
      ...
      <ul class="dropdown-menu dropdown-menu-left">
         ...
         <li id="frmMyform">
		  ...
		  <li id="div1">
            ...
            <a href="#">
              ...
              <?php echo 'Due to pricing'; ?>
            </a> <a href="#">
           
              <?php echo 'Better product by other Brand'; ?>
            </a> <a href="#">
       ...
       <?php
		   ...
		   if($lead->customer_new_existing=='ncustomer')
		   {
   ...
   <div class="row">
      <div class="lead-view<?php if(!isset($lead)){echo ' hide'; } ?>">
         <div class="col-md-4 col-xs-12 mtop15">
            ...
            <p class="text-muted lead-field-heading no-mtop"><?php echo _l('Customer Group'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $this->leads_model->customer_group_byname($lead->customer_group) != '' ? $this->leads_model->customer_group_byname($lead->customer_group) : '-') ?></p>
            
			<p class="text-muted lead-field-heading no-mtop"><?php echo _l('Customer'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $this->leads_model->get_customer_name($lead->customer_name) != '' ? $this->leads_model->get_customer_name($lead->customer_name) : '-') ?></p>
            
			<p class="text-muted lead-field-heading no-mtop"><?php echo _l('Lead Title'); ?></p>
			...
			<p class="text-muted lead-field-heading"><?php echo _l('Opportunity Amount(In Lacs)'); ?></p>
            <p class="bold font-medium-xs"><?php
  
			echo (isset($lead) &&  $lead->opportunity != '' ? $lead->opportunity : '-') ?></p>
				
			<p class="text-muted lead-field-heading"><?php echo 'Finalization Month (Expected)'; ?></p>
            <p class="bold font-medium-xs"><?php 
			$timestamp = strtotime($lead->accepacted_date);
			$date_lead = date("d-m-Y", $timestamp);		
			echo (isset($lead) && $lead->accepacted_date != '' ? $date_lead : '-') ?></p>

			<p class="text-muted lead-field-heading"><?php echo _l('Project Location'); ?></p>
            <p class="bold font-medium-xs"><?php
  
			echo (isset($lead) &&  $lead->project_location != '' ? $lead->project_location : '-') ?></p>
			
			<p class="text-muted lead-field-heading"><?php echo _l('Dealer Name'); ?></p>
            <p class="bold font-medium-xs"><?php
  
			echo (isset($lead) &&  $lead->dilar != '' ? $lead->dilar : '-') ?></p>
			<p class="text-muted lead-field-heading"><?php echo _l('Contrator Name'); ?></p>
            <p class="bold font-medium-xs"><?php
  
			echo (isset($lead) &&  $lead->contractor != '' ? $lead->contractor : '-') ?></p>
         ...
         <div class="col-md-4 col-xs-12 mtop15">
            <div class="lead-info-heading">
               <h4 class="no-margin font-medium-xs bold">
                  <?php echo _l('Lead Contact Details');?>
               </h4>
            </div>
			<p class="text-muted lead-field-heading"><?php echo _l('Contact Person'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->lead_name != '' ? $lead->lead_name : '-') ?></p>
            ...
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->city_name != '' ? $lead->city_name : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_state'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $this->staff_model->get_state_bycityname($lead->city_name) != '' ? $this->staff_model->get_state_bycityname($lead->city_name) : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('lead_country'); ?></p>
            <p class="bold font-medium-xs"><?php echo (isset($lead) && $lead->country != 0 ? $lead->country_name : 'India') ?></p>
         ...
         <div class="col-md-4 col-xs-12 mtop15">
			...
			<p class="text-muted lead-field-heading no-mtop"><?php echo _l('Interaction stage'); ?></p>
            <p class="bold font-medium-xs mbot15"><?php 
			$status_loss ='';
			if($lead->status==6 || $lead->status==7)
			{
			$status_loss=$this->leads_model->get_status_loss($lead->status_lost);
				
			}
			
			else{
			$status_loss =$lead->status_lost;
				
			}
			
			echo (isset($lead) && $lead->status_lost != '' ? $status_loss : '-') ?></p>
			<?php 
			if($lead->status==6){
			
			?>
			
			<p class="text-muted lead-field-heading no-mtop"><?php echo 'Total Amount(In Lacs)'; ?></p>
            <p class="bold font-medium-xs mbot15">
			<?php echo (isset($lead) && $lead->project_total_amount != '' ? $lead->project_total_amount : '-') ?>
			
			</p>
			<?php
			}else if($lead->status==7){
				?>
				<p class="text-muted lead-field-heading no-mtop"><?php echo ('Lead Awarded to'); ?></p>
            <p class="bold font-medium-xs mbot15">
			<?php echo (isset($lead) && $lead->project_awarded_to != '' ? $lead->project_awarded_to : '-') ?>
			...
			?>
			<p class="text-muted lead-field-heading no-mtop"><?php echo _l('Competition with'); ?></p>
            <p class="bold font-medium-xs mbot15"><?php echo (isset($lead) && $lead->competition != '' ? $lead->competition : '-') ?></p>
...
$timestamp = strtotime($lead->dateadded);
	$date = date("d-m-Y h:i A", $timestamp);


			echo (isset($lead) && $date != '' ? '<span class="text-has-action" data-toggle="tooltipu" data-title="'._dt($date).'">' .($date) .'</span>' : '-') ?></p>
            <p class="text-muted lead-field-heading"><?php echo _l('leads_dt_last_contact'); ?></p>
          
            <p class="bold font-medium-xs">
<?php if(isset($lead)){
                    if($lead->customer_type == 'A'){
                      echo 'A (Above 1 Cr)';
                    }
					
					elseif($lead->customer_type== 'B'){
	 echo 'B (Above 50 lakhs)';
	 
}
elseif($lead->customer_type== 'C'){
	 echo 'C (Above 10 lakhs)';
	
}
elseif($lead->customer_type== 'D'){
	 echo 'D (0 – 10 lakhs)';
            ...
            <p class="text-muted lead-field-heading"><?php echo _l('Can be called?'); ?></p>
            <p class="bold font-medium-xs mbot15">
               <?php if(isset($lead)){
                    if($lead->can_be_called == 'Y'){
   ...
   <?php if(isset($lead)){ ?>
   <div class="lead-latest-activity lead-view hide">
      <div class="lead-info-heading">
