...
function getstaffreportingto($userid = '')
{
	$name = '';
    $CI =& get_instance();
    
	$str_arr = explode (",", $userid); 
	foreach($str_arr as $staffr){
        $CI->db->where('staffid', $staffr);
        $staff = $CI->db->select('firstname,lastname')->from('tblstaff')->get()->row();
 ...
 */
function logActivity($activity_id='',$description, $staffid = '')
{
    $CI =& get_instance();
    $log = array(
...
function get_country($id)
{
    ...
    if (!$country) {
        $CI->db->where('id', $id);
        $country = $CI->db->get('tbl_country')->row();
        $CI->object_cache->add('db-country-'.$id, $country);
    }
