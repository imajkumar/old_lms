...
class Tasks_model extends CRM_Model
{
    ...
    public function add($data, $clientRequest = false)
    {
		...
		$this->db->where('id', $data['rel_id']);
        $assigned_person = $this->db->get('tblleads')->row()->assigned;
		
		$subject = "Halonix - Task Assigned";
		$message = "<html><head><title>HTML email</title></head><body>New Task Assigned by ".get_staff_full_name().' on date '.$data['startdate'];
		
		$message .= "</body></html>";
		$headers = "From: Halonix - Lead<no-reply@halonix.co.in>" . "\r\n";
		$headers .= "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
		$this->db->where('staffid', $assigned_person);
        $emailIDS = $this->db->get('tblstaff')->row()->email;
		
		mail($emailIDS, $subject, $message, $headers);
		
					
        $data['startdate'] = to_sql_date($data['startdate']);
        $data['duedate']   = to_sql_date($data['duedate']);
        $data['dateadded'] = date('Y-m-d H:i:s');
        $data['addedfrom'] = $clientRequest == false ? $assigned_person : get_contact_user_id();
		...
		$this->db->where('id', $data['rel_id']);
        $reportingto = $this->db->get('tblleads')->row()->reportingto;
        ...
        if ($insert_id) {
            ...
            if (isset($data['rel_type']) && $data['rel_type'] == 'lead') {
                ...
                $this->leads_model->log_lead_activity($data['rel_id'], 'Task Assigned', false, serialize(array(
            ...
            if ($clientRequest == false) {
                ...
                if ($new_task_auto_assign_creator == true) {
                    $this->db->insert('tblstafftaskassignees', array(
                        'taskid' => $insert_id,
                        'staffid' => $assigned_person,
                        'assigned_from'=>$assigned_person,
                    ));
                }
			...
			logActivity($insert_id,'New Task Added [ ID: ' . $insert_id . ' Description: ' . $data['name'] . ']');
