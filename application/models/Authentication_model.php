    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($user) {
                ...
                logActivity('Failed Login Attempt [Employee Code:' . $email . ', Is Staff Member:' . ($staff == true ? 'Yes' : 'No') . ', IP:' . $this->input->ip_address() . ']','');

                return false;
            }

            if ($user->active == 0) {
                logActivity('Inactive User Tried to Login [Employee Code:' . $email . ', Is Staff Member:' . ($staff == true ? 'Yes' : 'No') . ', IP:' . $this->input->ip_address() . ']','');
            ...
            if ($staff == true) {
                ...
                if (!$twoFactorAuth) {
                    ...
                    $user_data = array(
						...
						'staff_reporting_to' => $user->reporting_to,
                        'staff_state' => $user->state,
                        'staff_region' => $user->region,
