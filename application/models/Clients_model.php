...
class Clients_model extends CRM_Model
{
    ...
    public function __construct()
    {
        ...
        $this->contact_columns = do_action('contact_columns', array('firstname', 'lastname', 'email', 'phonenumber','mobilenumber','addedby', 'title', 'password', 'send_set_password_email', 'donotsendwelcomeemail', 'permissions', 'direction', 'invoice_emails', 'estimate_emails', 'credit_note_emails', 'contract_emails', 'task_emails', 'project_emails', 'is_primary'));

        $this->load->model(array('client_vault_entries_model', 'client_groups_model', 'statement_model', 'staff_model'));
    }
	...
	public function getlead_contact($id = '', $where = array())
    {
    ...
    public function add($data, $client_or_lead_convert_request = false)
    {
        ...
        if ($contact_data['addedby'] > 1) {
			...
			if ($staff) {
				$data['reportingto'] = $staff->reporting_to;
			} else {
				$data['reportingto'] = 0;
			}
        }else{
    ...
    public function update($data, $id, $client_request = false)
    {
		...
		if($data_customer_->vat != $data['vat']){
			$log_change .= '-> GST updated from "'.$data_customer_->vat. '" to '. $data['vat'].'<br>';
		}
		...
		if($data_customer_->valid_to != $data['valid_to']){
			$log_change .= '-> Valid to Date updated from "'.$data_customer_->valid_to. '" to '.$data['valid_to'].'<br>';
		}
		if($data_customer_->address != $data['address']){
			$log_change .= '-> Address updated from "'.$data_customer_->address. '" to '.$data['address'].'<br>';
		}
		if($data_customer_->website != $data['website']){
			$log_change .= '-> Website updated from "'.$data_customer_->website. '" to '.$data['website'].'<br>';
		}
		if($data_customer_->customer_type != $data['customer_type']){
			$log_change .= '-> Customer Type updated from "'.$data_customer_->customer_type. '" to '.$data['customer_type'].'<br>';
		}
		if($data_customer_->approve != $data['approve']){
			if($data['approve']==1){
				$log_change .= '-> Customer Approved'.'<br>';
			}else if($data['approve']==2){
				$log_change .= '-> Customer Resubmit'.'<br>';
    ...
    public function update_contact($data, $id, $client_request = false)
    {
        ...
        if ($affectedRows > 0 && !$set_password_email_sent) {
            logActivity($id,'Contact Updated [' . $data['firstname'] . ' ' . $data['lastname'] . ']');
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        if ($contact_id) {
            ...
            logActivity($data['userid'],'Contact Created [' . $data['firstname'] . ' ' . $data['lastname'] . ']');
    ...
    public function change_client_status($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            logActivity($id,'Customer Status Changed [CustomerID: ' . $id . ' Status(Active/Inactive): ' . $status . ']');
    ...
    */
	...
	public function no_of_cust_bydate($date,$staffid)
	{
