...
class xin_model extends CRM_Model {
	...
	public function get_state_byid($id)
	{
	   $this->db->select('state_name');
		...
		$this->db->where('id',$id);
		$reault_array = $this->db->get()->result_array();
		return $reault_array[0]['state_name'];
	}
	...
	public function site_title() {
		return 'Technix Infotech | Workable Zone';
	}
	...
	{
	  $query = $this->db->query("SELECT * from lead_status");
  	  return $query->result();
	}
	...
	{
	  $query = $this->db->query("SELECT * from lead_type");
  	  return $query->result();
	}
	...
	public function current_month_attendance() {
		...
		$session = $this->session->userdata('username');
		$query = $this->db->query("SELECT * from xin_attendance_time where attendance_date like '".$current_month."%' and `employee_id` = '".$session['user_id']."' ");
		return $query->num_rows();
	}
