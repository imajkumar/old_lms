...
class Utilities_model extends CRM_Model
{
         ...
         if (!$client_data) {
            ...
            foreach ($available_reminders as $key) {
                if (get_option('show_' . $key . '_reminders_on_calendar') == 1  && !$ff || $ff && array_key_exists($key.'_reminders', $filters)) {
                    if (get_staff_role() == 1) {
						$this->db->select('date,description,firstname,lastname,creator,staff,rel_id,tblreminders.state,tblreminders.reportingto')
						->from('tblreminders')
						->where('(date BETWEEN "' . $start . '" AND "' . $end . '")')
						->where('tblreminders.staff = "' . get_staff_user_id() . '"')
						->where('rel_type', $key)
					...
					else if(get_staff_role() == 7 || get_staff_role() == 4) 
					{
						...
						->from('tblreminders')
						->where('(date BETWEEN "' . $start . '" AND "' . $end . '")')
						->where('tblreminders.state IN('. get_staff_state_id() .')')
						->where('rel_type', $key)
						->join('tblstaff', 'tblstaff.staffid = tblreminders.staff');
						if ($hideNotifiedReminders == '1') {
							$this->db->where('isnotified', 0);
					...
					{
						$this->db->select('date,description,firstname,lastname,creator,staff,rel_id,state,reporting_to as reportto')
						->from('tblreminders')
						->where('(date BETWEEN "' . $start . '" AND "' . $end . '")')
						->where('reportto', '"%'.get_staff_user_id().'%"')
						->where('rel_type', $key)
						->join('tblstaff', 'tblstaff.staffid = tblreminders.staff');
						if ($hideNotifiedReminders == '1') {
							$this->db->where('isnotified', 0);
	...
	public function get_calendar_data1($start, $end, $client_id = '', $contact_id = '', $filters = '')
    {
        $is_admin                     = is_admin();
        $has_permission_tasks_view    = has_permission('tasks', '', 'view');
        $has_permission_projects_view = has_permission('projects', '', 'view');
		...
		if($filters==''){
			$array_cond = array('status !='=> 5);
		}else{
			$array_cond = array('status !='=> 5,'addedfrom'=> $filters);
		}
		
		/* 
		if(isset($filters['staff_id'])){
			$array_cond = array('status !='=> 5,'addedfrom'=> $filters['staff_id']);
		}else if(isset($filters['calendar_filters'])){
			$array_cond = array('status !='=> 5);
		} */
			
			$this->db->select('name as title,id,'.tasks_rel_name_select_query() . ' as rel_name,rel_id,status,CASE WHEN duedate IS NULL THEN startdate ELSE duedate END as date', false);
            $this->db->from('tblstafftasks');
            $this->db->where($array_cond);
