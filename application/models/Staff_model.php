...
class Staff_model extends CRM_Model
{
    ...
    public function delete($id, $transfer_data_to)
    {
        ...
        logActivity($id,'Staff Member Deleted [Name: ' . $name . ', Data Transferred To: ' . $transferred_to . ']');
     ...
     */
	 ...
	 public function list_staff_data($id = null) {
	...
	public function get_staff_data($id = null,$where = array()) {
	...
	public function get_by_role($id = '', $active = '', $where = array())
    {
        $select_str = '*,CONCAT(firstname," ",lastname) as full_name';
        ...
        $this->db->order_by('firstname', 'desc');
		$this->db->where(array('role' => '1', 'active' => '1'));
        return $this->db->get('tblstaff')->result_array();
    }
    ...
    public function add($data)
    {
        ...
        if ($staffid) {
            ...
            logActivity($staffid, 'New Staff Member Added [ID: ' . $staffid . ', ' . $data['firstname'] . ' ' . $data['lastname'] . ']');
    ...
    public function update($data, $id)
    {
		...
		//---------------- update lead ----------------//
			$data_lead['reportingto'] = $data['reporting_to'];
			$this->db->where('assigned', $id);
			$this->db->update('tblleads', $data_lead);
			
		
		//--------------- update customer ------------ //
			$this->db->where('addedfrom', $id);
			$this->db->update('tblclients', $data_lead);
        ...
        if ($affectedRows > 0) {
            ...
            logActivity($id, 'Staff Member Updated [ID: ' . $id . ', ' . $data['firstname'] . ' ' . $data['lastname'] . ']');
    ...
    public function update_profile($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            logActivity($id,'Staff Profile Updated [Staff: ' . get_staff_full_name($id) . ']');
    ...
    public function change_password($data, $userid)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            logActivity($userid,'Staff Password Changed [' . $userid . ']');
    ...
    public function change_staff_status($id, $status)
    {
        ...
        logActivity($id, 'Staff Status Changed [StaffID: ' . $id . ' - Status(Active/Inactive): ' . $status . ']');
    }
	public function get_role_data($id='')
    {
        $this->db->where('roleid', $id);

        return $this->db->get('tblroles')->row()->name;
	...
	public function get_staff_email()
    {
        ...
        return $this->db->get('tblstaff')->row()->email;
		
    }
	public function get_staff_email_byid($staff_id)
    {
        $this->db->where('staffid', $staff_id);
        return $this->db->get('tblstaff')->row()->email;
		
    }
	public function get_nsm_email()
    {
        $this->db->where('role', '8');
        return $this->db->get('tblstaff')->row()->email;
    }
	public function get_project_manager_email()
    {
	...
	public function get_assproject_manager_email()
    {
      	$this->db->select('email');
		$this->db->from('tblstaff');
		$this->db->like('role', '7');
	...
	public function get_reportingto($staff_id)
    {
        $this->db->where('staffid', $staff_id);
        return $this->db->get('tblstaff')->row()->reporting_to;
    }
	
	public function getEmailIDPM($field,$table,$column,$keyword) {
        $this->db->select('email');
		$this->db->from('tblstaff');
		//$where = "role='4' AND state LIKE '%".$keyword."%'";
		$where = "role='4'";
		$this->db->where($where);
		return $this->db->get()->result_array();
       
    }
	public function getEmailIDASM($field,$table,$column,$keyword) {
        $this->db->select('email');
		$this->db->from('tblstaff');
		//$where = "role='4' AND state LIKE '%".$keyword."%'";
		$where = "role='7'";
	...
	public function get_state_bycityname($cityname='')
    {
		$this->db->where('city', $cityname);
        $state_id = $this->db->get('tbl_city')->row()->state_id;
