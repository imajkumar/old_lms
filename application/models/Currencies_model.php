...
class Currencies_model extends CRM_Model
{
	...
	public function getCountry($id = false)
    {
        ...
        $country = $this->object_cache->get('currencies-data');

        if(!$country && !is_array($country)) {
            $country = $this->db->get('tbl_country')->result_array();
            $this->object_cache->add('currencies-data',$country);
        }

        return $country;
    }
	public function getStateByCountry($id = false)
    {
        if (is_numeric($id)) {
        ...
        $state = $this->object_cache->get('currencies-data');

        if(!$state && !is_array($state)) {
            $state = $this->db->get('tbl_state')->where('country_id', $id)->result_array();
            $this->object_cache->add('currencies-data',$state);
        }

        return $state;
    }
	public function getStateByRegion($id = false)
    {
        if (is_numeric($id)) {
            $this->db->where('region', $id);

            return $this->db->get('tblregion_state')->result_array();
        }

        $state = $this->object_cache->get('currencies-data');

        if(!$state && !is_array($state)) {
            $state = $this->db->get('tblregion_state')->where('region', $id)->result_array();
            $this->object_cache->add('currencies-data',$state);
        }

        return $state;
    }
	public function get_states_id($id = false)
    {
	...
	public function get_depot_code_list($id = null) {
	...
	public function list_region($id = null) {
        $this->db->select()->from('tblregion_state');
        if ($id != null) {
            $this->db->where('id', $id);
        } else {
            $this->db->order_by('id');
	 ...
	 public function getregionBystate($region_id) {
        $this->db->select('tblregion_state.state_id as state_id ,tbl_state.state as state_name')->from('tblregion_state');
		$this->db->join('tbl_state', 'tbl_state.id=tblregion_state.state_id');
		$this->db->where_in('region', $region_id);
	...
	public function getstaffByregion($region_id) {
        $regionids = implode(',',$region_id);	
        if(get_staff_role() >= 9  || is_admin())
		{				
			$arruser = "region IN(".$regionids.") AND is_not_staff = 0";
		}else{
			$arruser = "is_not_staff = 0 AND region IN(".$regionids.") AND ( CONCAT(',', reporting_to, ',')  LIKE '%, ".get_staff_user_id().",%'  OR CONCAT(',', reporting_to, ',')  LIKE '%,".get_staff_user_id().",%' ) AND is_not_staff = 0";
		}
	 ...
	 public function lists_region_data() {
        $this->db->select('tblregion_state.*,b.region as region_id');
        $this->db->from('tblregion_state');
        $this->db->join('tblregion b', 'b.id=tblregion_state.region');
