...
class Misc_model extends CRM_Model
{
    ...
    public function add_reminder($data, $id)
    {
        ...
        if ($insert_id) {
            if ($data['rel_type'] == 'lead') {
				...
				if($data['assigned_to'] !=''){
                $this->leads_model->log_lead_activity($data['rel_id'], 'Set a new Follow up for <strong> Technical with remark: '.$data['description'].' on date '._dt($data['date']).'"</strong>', false, serialize(array(
                    get_staff_full_name($data['staff']),
                    _dt($data['date'])
                    )));
				}else{
					$this->leads_model->log_lead_activity($data['rel_id'], 'Set a new Follow up for <strong> Self with remark: "'. $data['description'].' on date '._dt($data['date']).'"</strong>', false, serialize(array(
                    get_staff_full_name($data['staff']),
                    _dt($data['date'])
                    )));
				}
			...
			$notified = add_notification(array(
                    ...
                    'additional_data' => serialize(array(
                        $rel_values['name'] . ' - ' . strip_tags(mb_substr($data['description'], 0, 50)) . '...',
                    )),
                ));
    ...
    public function edit_reminder($data, $id)
    {
		...
		if (isset($data['technical_comment'])) {
            $data['technical_comment'] = nl2br($data['technical_comment']);
        }
        $data['description'] = nl2br($data['description']);
        $data['assigned_to']     = $data['assigned_to'];
		$data['reminder_status']   = $data['reminder_status'];
       ...
       if($this->db->affected_rows() > 0){
		   ...
		   if($data['technical_comment'] !=''){
				$description = $data['technical_comment'];
			    $notified1 = add_notification(array(
                    'fromcompany' => true,
                    'touserid' => $data['staff'],
                    'description' => 'not_update_reminder_for',
                    'link' => $notificationLink,
                    'additional_data' => serialize(array(
                        $rel_values['name'] . ' - ' . strip_tags(mb_substr($data['technical_comment'], 0, 50)) . '...',
                    )),
                ));

				if ($notified1) {
					$notifiedUsers = array();
					array_push($notifiedUsers, $data['staff']);
					//pusher_trigger_notification($notifiedUsers);
				}
		   }else{
			...
			$this->load->model('leads_model');
			$this->leads_model->log_lead_activity($data['rel_id'], 'Meeting / Follow Up Updated :- <strong>"'.$description.' on date '._dt($data['date']).'"</strong>', false, serialize(array(
				get_staff_full_name($data['staff']),
				_dt($data['date'])
				)));
            
			logActivity($data['rel_id'],'Meeting / Follow Up Updated [' . ucfirst($data['rel_type']) . 'ID: ' . $data['rel_id'] . ' Description: ' . $description . ']');
            return true;
       }
	...
	public function get_activity_log_client($client_id)
    {
        $this->db->where('activity_id', $client_id);
        $this->db->order_by('date', 'desc');
    ...
    public function delete_reminder($id)
    {
        ...
        if ($reminder->creator == get_staff_user_id() || is_admin()) {
            ...
            if ($this->db->affected_rows() > 0) {
                logActivity($id,'Reminder Deleted [' . ucfirst($reminder->rel_type) . 'ID: ' . $reminder->id . ' Description: ' . $reminder->description . ']');
