...
class Dashboard extends Admin_controller
{
    ...
    public function index()
    {
		...
		$data['lead'] = $this->db->get_where('tblleads', array('id' => $id))->row();
		...
		$data['customer_status_stats']                = json_encode($this->dashboard_model->customer_status_stats());
		...
		if (get_staff_role() == 0 || get_staff_role() > 8) {
            $data['staffmember'] = $this->staff_model->get_staff_data('', array('is_not_staff'=>0));
        } else if (get_staff_role() > 1) {
            $data['staffmember'] = $this->staff_model->get_staff_data('', array(
