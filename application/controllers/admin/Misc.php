...
class Misc extends Admin_controller
{
	...
	public function run_cron_lmsdaily()
    {
        if (is_admin()) {
            $this->load->model('cron_model');
            $this->cron_model->run_lmsdaily(true);
            redirect(admin_url('settings?group=reportemail'));
        }
    }
	public function run_cron_stage()
    {
        if (is_admin()) {
            $this->load->model('cron_model');
            $this->cron_model->run_stage(true);
            redirect(admin_url('settings?group=reportemail'));
        }
    }
	public function run_cron_stagesummary()
    {
        if (is_admin()) {
            $this->load->model('cron_model');
            $this->cron_model->run_stagesummary(true);
            redirect(admin_url('settings?group=reportemail'));
        }
    }
	
	public function run_cron_winloss()
    {
        if (is_admin()) {
            $this->load->model('cron_model');
...
/* Since version 1.0.2 add client reminder */
    public function update_reminder($rel_id, $rel_type)
    {
        $message    = '';
        $alert_type = 'warning';
		if ($this->input->post()) {
            $success = $this->misc_model->edit_reminder($this->input->post(),$rel_id);
            if ($success) {
