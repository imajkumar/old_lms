...
class Utilities extends Admin_controller
{
    ...
    function test(){
		 $success = $this->utilities_model->test();
		 print_r($success);
	}
    /* Calendar functions */
	...
	public function calendar_reminder()
    {
        if ($this->input->post()) {
            ...
            /* $data    = $this->input->post();
            $success = $this->utilities_model->add_reminder($data);
            $message = '';
            if ($success) {
                if (isset($data['eventid'])) {
                    $message = _l('event_updated');
		...
		//redirect('admin/utilities/calendar');
        
    }
	public function calendar_form_data()
    {
        if ($this->input->post('calendar_data_type')=='calendar_meeting') {
            ...
            if ($success) {
                if (isset($data['eventid'])) {
                    ...
                    $message = _l('utility_calendar_meeting_added_successfully');
                }
            }
            echo json_encode(array(
                'success' => $success,
            ...
            die();
        }if ($this->input->post('calendar_data_type')=='calendar_task') {
			$data    = $this->input->post();
            $success = $this->utilities_model->add_task($data);
            $message = '';
            if ($success) {
                if (isset($data['eventid'])) {
                    $message = _l('event_updated');
    ...
    {
        $data['meeting'] = $this->utilities_model->get_meeting($id);
        $this->load->view('admin/utilities/meeting', $data);
    }
