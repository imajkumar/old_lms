...
class Clients extends Admin_controller
{
    ...
    {
        parent::__construct();
        $this->not_importable_clients_fields = do_action('not_importable_clients_fields',array('userid', 'id', 'is_primary', 'password', 'datecreated', 'last_ip','addedfrom', 'last_login', 'last_password_change', 'active', 'new_pass_key', 'new_pass_key_requested', 'leadid', 'default_currency', 'profile_image', 'default_language', 'direction', 'show_primary_contact', 'invoice_emails', 'estimate_emails', 'project_emails', 'task_emails', 'contract_emails', 'credit_note_emails','last_active_time'));
    ...
    public function index()
    {
		...
		if (get_staff_role() == 0 || get_staff_role() > 8) {
			...
			$where = "is_not_staff=0 AND FIND_IN_SET('".get_staff_user_id()."', reporting_to) > 0";
            $data['staff'] = $this->staff_model->get_staff_data('', $where);
        }
	...
	public function GetGroupName(){
        ...
        $data=$this->clients_model->GetRow($keyword);        
        echo json_encode($data);
    }
	
	public function GetcompanyName(){
        $keyword=$this->input->post('keyword');
        $data=$this->clients_model->GetRowcompany($keyword);        
        echo json_encode($data);
    }
    ...
    /* public function client($id = '')
    {
        ...
        if ($this->input->post()) {
			if ($id == '') {
				...
				$query = $this->db->query('SELECT * FROM tblclients where company = "'.$data['company'].'"');
				
				if($query->num_rows() > 0){
					//set_alert('danger', 'Record Already Exist!');
				}
				else if($query->num_rows() == 0){
					$data['reportingto'] = get_staff_reporting_to();
					$save_and_add_contact = 0;
					if (isset($data['save_and_add_contact'])) {
						unset($data['save_and_add_contact']);
						$save_and_add_contact = 1;
					}
		...
		$this->load->model('leads_model');$this->load->model('staff_model');
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $data['template'] = get_email_template_for_sending($template_name, $email,$sta_email);
 ...
 */
  ...
  /* Edit client or add new client*/
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
				...
				if ($id == '') {
					...
					$isexist =  $this->db->get('tblclients')->row()->company;
			
					if($isexist == $data['company']){
    ...
    public function change_contact_is_primary($id, $status)
    {
        /* if (has_permission('customers', '', 'edit') || is_customer_admin(get_user_id_by_contact_id($id))) { */
            if ($this->input->is_ajax_request()) {
                $this->clients_model->change_contact_is_primary($id, $status);
	...
	public function change_client_approve($id, $status)
    {
        if ($this->input->is_ajax_request()) {
            $this->clients_model->change_client_approve($id, $status);
        }
 ...
 public function import()
    {
        ...
        $state_fields = array('state');
        $city_fields = array('city');
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        foreach ($rows as $row) {
                            ...
                            for ($i = 0; $i < count($db_fields); $i++) {
                                ...
                                if ($db_fields[$i] == 'addedby') {
                                    $this->db->where('emp_code', $row[$i]);
                                ...
                                if ($db_fields[$i] == 'datetime') {
									$row[$i] = date('Y-m-d H:i:s');
                                }
								
                                if ($db_fields[$i] == 'email') {
                                    $addedby = total_rows('tblcontacts', array('email' => $row[$i]));
                                    // don't insert duplicate emails
                                    if ($email_exists > 0) {
                                        $duplicate = true;
                                    }
                                ...
                                if (in_array($db_fields[$i], $required) && $row[$i] == '' && $db_fields[$i] != 'company') {
									...
									if (in_array($db_fields[$i], $country_fields)) {
										if ($row[$i] != '') {
											if (!is_numeric($row[$i])) {
												$this->db->where('country', $row[$i]);
												$country = $this->db->get('tbl_country')->row();
												if ($country) {
													$row[$i] = $country->id;
												} else {
													$row[$i] = 0;
									...
									}elseif (in_array($db_fields[$i], $state_fields)) {
										if ($row[$i] != '') {
											if (!is_numeric($row[$i])) {
												$this->db->where('state', $row[$i]);
												$state = $this->db->get('tbl_state')->row();
												if ($state) {
													$row[$i] = $state->id;
												} else {
													$row[$i] = 0;
									...
									elseif (in_array($db_fields[$i], $city_fields)) {
										if ($row[$i] != '') {
											if (!is_numeric($row[$i])) {
												$this->db->where('city', $row[$i]);
												$city = $this->db->get('tbl_city')->row();
												if ($city) {
													$row[$i] = $city->id;
												} else {
													$row[$i] = 0;
    ...
    public function group()
    {
        ...
        if ($this->input->is_ajax_request()) {
			...
			$isexist =  $this->db->get('tblcustomersgroups')->row()->name;
