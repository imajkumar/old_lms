...
class Staff extends Admin_controller
{
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
			...
			$state_data = $this->input->post('state');
			 ...
			 $region_data = $this->input->post('region');
			 ...
			 $depot_code_data = $this->input->post('depot_code');
			
			 foreach($depot_code_data as $row)
			 {
			  $depot_code .= $row . ', ';
			 }
			 $depot_code = substr($depot_code, 0, -2);
			
			 $data['depot_code'] = $depot_code;
			
			 
			 
			 $data['reporting_to'] = $this->input->post('reporting_to').','.$this->input->post('reporting_manager');
		
			$query_rep = $this->db->query('SELECT reportingto FROM tblleads where reportingto LIKE ("%'.$id.'%")');
			$reportingto = $query_rep->row()->reportingto;
		
			$newreportingto = $reportingto.','.$this->input->post('reporting_manager');
			
			$this->db->where('reportingto', $reportingto);
        ...
        if ($id == '') { 
			...
			$member_list= $this->staff_model->list_staff_data($id);
        ...
        $data['stateList'] = $this->currencies_model->getStateByRegion("104");
		$data['region_list'] = $this->currencies_model->get_region();
		
		$data['depot_code_list'] = $this->currencies_model->get_depot_code_list();
		 
		$data['member_list']= $this->staff_model->list_staff_data($id);
	...
	public function getStaffByZone()
    {
        $zone = $this->input->get('zone');
        $this->db->select('*');
        $this->db->from('tblstaff');
        $condt = "region LIKE('%".$zone."%') AND reporting_to LIKE '%".get_staff_user_id()."%' AND active = 1";
		if(get_staff_role() < 9)
		$this->db->where($condt);
	...
	public function getStaffByZoneLead()
    {
        $zone = $this->input->get('zone');
        $this->db->select('*');
        $this->db->from('tblstaff');
		...
		$where = "assigned = '".$staffd["staffid"]."'";
								...
								if($ifzsmleadvalue > 0)
								{
									$data_staff[]= array('staffid'=>$staffd["staffid"],'firstname' => $staffd["firstname"],'lastname'=>$staffd["lastname"],'emp_code'=>$staffd["emp_code"]);
								}
